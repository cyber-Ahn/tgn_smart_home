...
def ini():
	...
	try:
		f = open("/home/pi/tgn_smart_home/config/themes.config","r")
    		...
    		print("cannot open themes.config")
	else:
		data = []
		for line in f:
			data.append(line)
	...
	ver = ""
	while index < 5:
		cach = read_eeprom(1,0x54,0x00,start_add_I)
		if cach != "X":
			ver = ver + cach
		index = index + 1
...
def callback21():
	...
	if backlight == 1:
		...
		mylcd.backlight(1)
def callback22():
	setn = "lxterminal -e python3 /home/pi/tgn_smart_home/libs/settings.py intsall_rom"
	os.system(setn)
def callback23():
...
setmenu.add_command(label="Cam Settings", command=callback23)
setmenu.add_command(label="Reset eeprom", command=callback22)
