		...
		def change_value_the_time():
			...
			if newtime != the_time:
				...
				if backlight == 0 and LCDpower == 1:
					mylcd.backlight(0)
				the_time= pcf8563ReadTimeB()+"\n"+textcpu+str(getCpuTemperature())+"°C\n"+stats
			...
			if newtime != the_time:
				...
				if is_connected(REMOTE_SERVER)=="Online":
					if allowed_key(openweatherkey) == "yes":
						...
						output = output+(dataText[32].rstrip())+str(data['sunset'])+'\n'
						#output = output+(dataText[33].rstrip())+str(data['icon'])+'\n'
						output = output+(dataText[34].rstrip())+str(data['dt'])+'\n'
						output = output+'---------------------------------------\n'
						output = output+temp_data+" / "+str(readLight())+'Lux\n'
						#output = output+get_BMP085()+'\n'
						global weather_t
						global weather_c
						global weather_w
						global weather_h
...
setmenu.add_command(label="PW on/off", command=callback37)
...
rommenu.add_command(label=(data[106].rstrip()), command=callback36)
